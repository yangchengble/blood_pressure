...
void app_alt_pair_store_bond_data(void)
{
    if (HAS_EEPROM)
    {
        ...
        {
            // is it required? if keys are the same then it shouldn't be...
            if ( (app_env.peer_addr_type == ADDR_RAND) && ((app_env.peer_addr.addr[5] & 0xC0) != 0xC0) )
    ...
    if (HAS_EEPROM)
    {
        ...
        for (i = 0; i < MAX_BOND_PEER; i++)
        {
                    ...
                    && (tmp_sec_env.auth & GAP_AUTH_BOND))
            {
                ...
                memcpy(&app_sec_env.peer_addr.addr, app_env.peer_addr.addr, BD_ADDR_LEN);   // update address (for random address devices)
...
bool app_alt_pair_get_next_bond_data(bool init)
{
    if (HAS_EEPROM)
    {
        ...
        do
        {
            ...
            if (tmp_sec_env.peer_addr_type == ADDR_PUBLIC && tmp_sec_env.auth & GAP_AUTH_BOND) //used entry (public address)
